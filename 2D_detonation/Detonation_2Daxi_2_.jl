...
function ExplosiveSolver(nr::Int, nz::Int, r_min::Float64, r_max::Float64,
    ...
    # Create explosive states with more conservative parameters
    # Reduced energy release and reaction rate for stability
    murnaghan = MurnaghanEOS(4.0e9, 8.0, 1630.0, 0.0)  # Softer: 4 GPa, K'=8
    jwl = JWLEOS(1.0e11, 1.0e9, 4.15, 0.95, 0.3, 1630.0, 2.0e6)  # Lower A,B,Q for stability
    
    explosive_state = Matrix{ExplosiveState}(undef, nr, nz)
    for j in 1:nz, i in 1:nr
        explosive_state[i,j] = ExplosiveState(0.0, murnaghan, jwl, -1.0, 5.0e5)  # Slower reaction
    end
...
function plot_explosive_solution(solver::ExplosiveSolver, t::Float64)
    ...
    try
        ...
        return plot(title="Plotting failed: $e")
    end
end

# More conservative explosive simulation parameters
function solve_explosive_detonation(nr::Int=25, nz::Int=20, t_final::Float64=20e-6)
    ...
    # Initiate detonation at center with delayed start
    initiate_detonation!(solver, 0.0, 0.05, 10e-6)  # Start at t=10μs for stability
    ...
    while t < t_final && step < 10000
        ...
        if step % 25 == 0
            ...
            for j in 1:solver.nz
                for i in 1:solver.nr
                    U = solver.U[:, i, j, solver.current]
                    if any(.!isfinite.(U)) || U[1] <= 0
                        println("ERROR: Invalid values at step $step, cell ($i,$j): $U")
                        all_finite = false
                        break
                    end
                end
            ...
            if !all_finite
                println("Simulation stopped due to numerical instability")
                break
            end
            
            # Sample some values
            center_i, center_j = solver.nr÷2, solver.nz÷2
            U_sample = solver.U[:, center_i, center_j, solver.current]
            state_sample = solver.explosive_state[center_i, center_j]
            
            if all(isfinite.(U_sample)) && U_sample[1] > 0
                W_sample = conservative_to_primitive_explosive(U_sample, state_sample)
                println("Step: $step, Time: $(round(t*1e6, digits=2)) μs, dt: $(round(solver.dt*1e9, digits=2)) ns")
                println("  Center: ρ=$(round(W_sample[1], digits=1)), p=$(round(W_sample[4]/1e6, digits=1)) MPa, λ=$(round(state_sample.lambda, digits=3))")
            else
                println("Step: $step, Time: $(round(t*1e6, digits=2)) μs - WARNING: Invalid center values")
            end
        end
    end
