...
function update_solution!(solver::ExplosiveSolver)
    ...
    # Special treatment for axis cells (i=1)
    for j in 2:solver.nz-1
        # At axis (r=0), use L'Hôpital's rule for geometric source terms
        # The source term -p/r becomes -2*dp/dr at r=0
        
        U_axis = solver.U[:, 1, j, current]
        U_right = solver.U[:, 2, j, current]
        cell_axis = solver.cell_states[1, j]
        cell_right = solver.cell_states[2, j]
        
        # Get pressures
        p_axis = pressure(cell_axis, U_axis[1], U_axis[4], U_axis[2], U_axis[3])
        p_right = pressure(cell_right, U_right[1], U_right[4], U_right[2], U_right[3])
        
        # Estimate dp/dr at axis
        dp_dr = (p_right - p_axis) / solver.dr
        
        # z-direction flux (normal treatment)
        U_B = solver.U[:, 1, max(j-1, 1), current]
        U_T = solver.U[:, 1, min(j+1, solver.nz), current]
        cell_B = solver.cell_states[1, max(j-1, 1)]
        cell_T = solver.cell_states[1, min(j+1, solver.nz)]
        
        flux_z = hlle_flux_mixed(U_B, U_T, cell_B, cell_T, 2)
        solver.U[:, 1, j, next] -= (solver.dt / solver.dz) * flux_z / 2.0
        
        # r-direction flux (one-sided due to symmetry)
        flux_r = hlle_flux_mixed(U_axis, U_right, cell_axis, cell_right, 1)
        solver.U[:, 1, j, next] -= (solver.dt / solver.dr) * flux_r
        
        # Axisymmetric source at axis: -2*dp/dr instead of -p/r
        source = [0.0, -2.0 * dp_dr, 0.0, 0.0]
        solver.U[:, 1, j, next] += solver.dt * source
        
        # Enforce symmetry: radial velocity must be zero at axis
        solver.U[2, 1, j, next] = 0.0
        
        # Ensure positivity
        solver.U[1, 1, j, next] = max(solver.U[1, 1, j, next], 1e-12)
        solver.U[4, 1, j, next] = max(solver.U[4, 1, j, next], 1e-12)
    end
    ...
    # Boundary conditions
    for j in 1:solver.nz
        # r boundaries - axis is already handled above
    ...
    # Additional safety: ensure axis symmetry is maintained
    for j in 1:solver.nz
        solver.U[2, 1, j, next] = 0.0  # Zero radial velocity at axis
    end
...
function run_simulation(; nr=40, nz=40, 
                       r_max=0.03, z_max=0.03,  # Smaller 3cm domain
                       charge_radius=0.005,     # 0.5cm charge
                       det_velocity=7000.0,     # Slightly lower det velocity
                       cfl=0.2,                 # More conservative CFL
                       t_final=1e-5,            # 10 μs
                       plot_interval=2e-6)      # Plot every 2 μs
